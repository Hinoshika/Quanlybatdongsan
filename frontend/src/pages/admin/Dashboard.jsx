...
export default function Dashboard() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [td, ct, csh, bd, yc] = await Promise.all([
      ...
      setYeuCauStats({
        tong: yeuCau.length,
        choXuLy: yeuCau.filter((x) => x.trang_thai === "CHO_XU_LY").length,
        hoanthanh: yeuCau.filter((x) => x.trang_thai === "HOAN_THANH").length,
        tuChoi: yeuCau.filter((x) => x.trang_thai === "TU_CHOI").length,
      });
    } catch (err) {
      console.error(err);
    } finally {
    ...
    >
      ...
      {loading ? (
        ...
        <>
          ...
          >
            <Title level={4}>📨 Thống kê yêu cầu</Title>
          ...
          >
            <Title level={5}>Tỷ lệ xử lý yêu cầu</Title>

            <Progress
              percent={
                yeuCauStats.tong > 0
                  ? Math.round(
                      ((yeuCauStats.hoanthanh + yeuCauStats.tuChoi) * 100) /
                        yeuCauStats.tong,
                    )
                  : 0
              }
            ...
            <div style={{ marginTop: 10 }}>
              Đã xử lý: {yeuCauStats.hoanthanh + yeuCauStats.tuChoi}/{" "}
              {yeuCauStats.tong}
            </div>
          </Card>
          {/* Insights Section */}
          ...
          >
            ...
            <Row gutter={24}>
              <Col span={12}>
                ...
                >
                  {stats.congTrinh > 0
                    ? ((stats.congTrinh / stats.thuaDat) * 100).toFixed(1)
                    : 0}
                  %
                </div>
              </Col>
